...
const FormatterPricePreview = ({
    ...
    elementsCol.forEach((col) => {
      elementsRow.forEach((row) => {
        ...
        if (commonElements.length == 1) {
          cells.push(
            <div key={`single_element_${col}_${row}`} className={style.cell}>
  ...
  return (
    <div
      ...
      ref={refGrid}>
      <CrossedLine ref={refCrossedLine} font={font} strikethrough={strikethrough} gridSize={gridSize} />
      {renderCells()}
    </div>
  );
};
